...
router.get('/kyc/status', (req, res) => {
  ...
  if (user && user.kyc_status && user.kyc_status !== 'none') {
    application = db.prepare(`
      SELECT id, company_name, country, legal_person_name, legal_person_id,
             contact_name, contact_phone, email, business_license, remark,
             reg_cert_file, id_card_file, status, reject_reason, created_at, updated_at
...
router.post('/kyc/submit', (req, res) => {
  const { company_name, country, legal_person_name, legal_person_id,
          contact_name, contact_phone, email, business_license, remark,
          reg_cert_file, id_card_file } = req.body;

  if (!company_name || !contact_name || !contact_phone) {
    return res.status(400).json({ code: 400, msg: '企业名称、联系人、联系电话为必填项' });
  }
  if (!country) {
    return res.status(400).json({ code: 400, msg: '请选择国家/地区' });
  }
  if (!legal_person_name || !legal_person_id) {
    return res.status(400).json({ code: 400, msg: '法人姓名和身份证号为必填项' });
  }
  ...
  const r = db.prepare(`
    INSERT INTO kyc_applications
      (user_id, company_name, country, legal_person_name, legal_person_id,
       contact_name, contact_phone, email, business_license, remark,
       reg_cert_file, id_card_file, status, submitter_ip)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?)
  `).run(req.user.id, company_name, country, legal_person_name, legal_person_id,
         contact_name, contact_phone, email || '', business_license || '', remark || '',
         reg_cert_file || '', id_card_file || '', req.ip || '');
