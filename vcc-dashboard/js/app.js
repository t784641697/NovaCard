  ...
  area.innerHTML = `
    ...
    <div id="applyStep2" class="hidden">
      ...
      <div class="panel">
        ...
        <div class="form-row col2">
          <div class="form-group" style="grid-column:span 2"><label>地址行 1*</label><input class="form-control" id="ap_addr1" placeholder="123 Main St"/></div>
          <div class="form-group" style="grid-column:span 2"><label>地址行 2</label><input class="form-control" id="ap_addr2" placeholder="Apt 4B（可选）"/></div>
          <div class="form-group"><label>城市</label><input class="form-control" id="ap_city" placeholder="San Jose"/></div>
          <div class="form-group"><label>州/省</label><input class="form-control" id="ap_state" placeholder="CA"/></div>
          <div class="form-group"><label>国家</label><input class="form-control" id="ap_country" placeholder="US" value="US"/></div>
          <div class="form-group"><label>邮政编码</label><input class="form-control" id="ap_postcode" placeholder="95131"/></div>
        </div>
        ...
        <div class="form-row col2">
          ...
          <div class="form-group"><label>单笔限额</label><input class="form-control" type="number" id="ap_single" placeholder="0"/></div>
          <div class="form-group"><label>日限额</label><input class="form-control" type="number" id="ap_day" placeholder="0"/></div>
          <div class="form-group"><label>月限额</label><input class="form-control" type="number" id="ap_month" placeholder="0"/></div>
        </div>
...
async function submitApply() {
  ...
  const payload = {
    ...
    card_address:{
      address_line_one: g('ap_addr1'),
      address_line_two: g('ap_addr2'),
      city:    g('ap_city'),
      state:   g('ap_state'),
      country: g('ap_country')||'US',
      post_code: g('ap_postcode'),
    }
  };
...
function renderCmSearchBar() {
  ...
  return '<div class="cm-search-section">'
    ...
    + '<button class="' + tabCls('frozen') + '" onclick="cmSelectTab(this,\'frozen\')">冻结</button>'
    + '<button class="' + tabCls('cancelled') + '" onclick="cmSelectTab(this,\'cancelled\')">已取消</button>'
    + '</div>'
    // 日期选择器前标签
...
async function crLoad() {
  ...
  wrap.innerHTML = '<div class="table-wrap"><table class="data-table"><thead><tr>' +
    '<th>ID</th><th>申请人</th><th>卡产品</th><th>持卡人</th><th>金额</th><th>状态</th><th>申请时间</th><th>操作</th>' +
    '</tr></thead><tbody>' +
    list.map(a => {
