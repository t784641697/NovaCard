...
> ⚠️ **此规则被 v1.0.99.3 推翻**：上游实际上不允许带余额 active 卡删除（错误码 700013）。详见 v1.0.99.3 段。
...
### 🐛 修复 bug（用户真实卡 5258……3750 / XR2067511181878833152 删卡失败截图反馈）

**现象**：toast 显示 `❌ 上游删卡失败:上游删卡失败:第三方获取数据失败`（"上游删卡失败" 重复了两次）
...
- 上游 vmcardio 返回 `{"code":700013,"msg":"第三方获取数据失败"}`
- SDK `vmcardioSDK.js:60-63` 抛错：`err.message='第三方获取数据失败'` `err.vmCode=700013`
- **真实原因**：~~vmcardio 上游不允许带余额 active 卡删除~~ ⚠️ **v1.0.99.4 推翻此假设**：见下文，700013 实际是 G5554LC (VC102) 卡段上游 deleteCard 端 bug/限制，S5258LL 卡段带 $20 余额也能成功删除。

### ⚠️ 业务规则更正
> v1.0.99.1 假设**被推翻（实测）**：vmcardio 上游 `deleteCard` API **对部分卡段**返回 700013（G5554LC/VC102），**其他卡段带余额正常删**（S5258LL $20 成功）。
...
- **前端** `vcc-dashboard/app.html:4703-4705` 700013 提示：
  - 从「上游可能因卡内仍有余额而拒绝删除...」
  - 改为「**G5554LC (VC102) 卡段上游 deleteCard 端存在限制**（错误码 700013），S5258LL 等其他卡段可正常删除。**该卡段暂时无法通过此系统删除**，请提供以下信息联系 vmcardio 上游客服：错误码 700013 / card_id XR2067511181878833152 / 卡段 G5554LC」
  - 不再引导用户去"消耗余额"（无意义）
...
## v1.0.99.5 删卡余额自动退款 (2026-06-24)

### 业务规则彻底澄清（三阶段真相演进）

| 版本 | 假设 | 真相 |
|------|------|------|
| v1.0.99.1 | 用户口述"上游自动退余额到用户账户" | 错：上游退到 vmcardio 平台账户，**不是**用户账户 |
| v1.0.99.3 | 我猜"上游不允许带余额删卡" | 错：S5258LL 带余额也能删，差异在**卡段**不在余额 |
| **v1.0.99.5** | **上游退到 vmcardio 平台账户** + **我们**需主动退给用户 | ✅ 正确 |

### 时间线证据（生产 logs/app.log）
- 03:55:58 商户余额 $50.19
- 03:56:52 S5258LL 卡 deleteCard 成功（带 $20）
- 04:02:28 商户余额 $70.19（+ $20）
- 04:10:46 cardDetail 同步，available_amount 清零
- 用户账户余额 $36.40 没动（v1.0.99.5 部署前）

### 修复
- 后端 `src/routes/cards.js:631-650`：上游 deleteCard 成功后调 `BalanceService.recordRefund(userId, amount, feeType, 0, desc, refId=card_id)` 主动退给用户
- 后端 `src/services/balanceService.js:147-187`：`recordRefund` 加 `refId` 参数
- 前端 `vcc-dashboard/app.html:4715-4717`：删卡成功 toast 改"余额 $X 已退到账户"
- 测试 12/12 全过（3.6a/3.6b/3.6c 新增验证退款流水）
- 手工追回 user_id=3 账户 +$20.00（5258 卡删卡）
...
之前 description 字段拼了 `5258...3750` 但**只在描述里**（不显眼），用户希望有独立列。

### 数据建模
- `transactions.ref_id` 字段早已在 v1.0.94 引入，存的是 `cards.card_id`（TEXT）
- 直接 `LEFT JOIN cards ON cards.card_id = transactions.ref_id` 拿 `card_number`/`product_code`/`label`
- 未关联 ref_id 的流水（管理员充值等）显示 `—`

### 改动汇总
| 改动 | 位置 |
|------|------|
| `/api/ledger` SQL 加 LEFT JOIN cards + t. 别名 | `src/routes/ledger.js:37-52` |
| `/api/ledger/export.csv` 加"关联卡号"列 | `src/routes/ledger.js:101-138` |
| admin `/users/:id/transactions` walletRows 同步 | `src/routes/admin.js:1413-1440` |
| 前端用户端表格 6 列→7 列 + masked `**** **** **** 3750` + 可点击跳详情 | `vcc-dashboard/app.html:4767-4775` + 4886-4895 |
| 前端 admin 端用户流水表升级 masked + 可点击 | `vcc-dashboard/app.html:5826-5830` |
| 新增工具函数 | `vcc-dashboard/app.html:4913-4935` |

### Bug 修复
1. `ambiguous column name: user_id` → where 加 `t.` 前缀
2. `no such column: t.user_id`（COUNT 子查询） → COUNT 那条也加 `t` 别名
3. nodemon 自动 reload 后测试通过

### 验证
- `scripts/v1.0.99.6_ledger_card_number_test.js` **12/12 全过**
...
- 代码 commit：`a07c8ca`
- 文档 commit：`4b444fe`
- 备份：`NovaCard-20260624_140938-V1.0.99.6.tar.gz` (10.8MB)
- 生产 HEAD：`4b444fe`（已 push origin/main，待手动 `git reset --hard` + `pm2 reload`）

## v1.0.99.7 历史 ref_id backfill (approved 申请) — 2026-06-24

**用户反馈**：v1.0.99.6 部署后只有 5258 卡有"关联卡号"记录，其他 9 条历史交易都是 `—`。

**根因**：v1.0.99.5 之前的代码**从没写过 `transactions.ref_id = card_id`**。只有 v1.0.99.5 删卡退还时**新增**了写 ref_id 的逻辑（`BalanceService.recordRefund(..., refId)`），所以只有 5258 卡有一条记录。

**方案**：后端 backfill 脚本 `scripts/v1.0.99.7_backfill_ledger_ref_id.js`，找该用户**最近一条** `status='approved'` 申请，写 `ref_id = card_id`。支持 `--dry-run` / `--apply`。

**设计决策**：**16 位卡号才能唯一标识一张卡**（VC113/G5554LC 等卡段名不够 —— 一人可开多张同卡段卡）。找不到对应申请 → 保持空，前端显示 `—`，**不伪造不解析 description**。

**生产 apply 3 条** (id=17/20/21)：XR2067511181878833152 + 2069455464522190849。

**代码 commit**：`3703da6` (含 fixup autosquash) + 部署 `48a9f65`。

## v1.0.99.8 申请被驳回显示"未开卡成功" — 2026-06-24

**用户反馈**：3 条 14:54-14:55 的流水是申请被驳回的（冻结开卡费 + 最低充值 + 释放资金），关联卡号显示 "VC113"。用户希望**驳回场景**显示特殊标记。

**方案**：约定式 ref_id 标识
- backfill 脚本扩展：同时匹配 `approved` + `rejected` 申请
- rejected → `ref_id = 'app_rejected:${app_id}:${product_code}'`（如 `app_rejected:4:VC113`）
- 前端 `formatLedgerCardCell` 新增 path 1.5：正则匹配 → 显示**黄/橙 "未开卡成功"** badge + pulse 动效

**三态判断**：(1) 真卡 (card_id) → masked 16位 + 可点击 (2) `app_rejected:*` → "未开卡成功" badge (3) 空 → `—`

**生产 apply 3 条** (id=16/18/19) → 全部写入 `app_rejected:4:VC113`。

**代码 commit**：`e1c57ca`。

## v1.0.99.9 移除账户流水页标题区 — 2026-06-24

**用户反馈**：圈红位置是 `<div class="page-header">` 块（"账户流水" + "账户余额变动历史记录" 标题副标题），希望移除。

**修复**：删除 `vcc-dashboard/app.html` line 4750-4752。

**代码 commit**：`755e04e`。

## v1.0.99.10 充值后异步同步余额 + deploy 脚本修复 — 2026-06-24

**用户反馈**：充值 10 美元后绿色 toast 成功，**上游=40 + admin 后台=40** 但**普通用户后台=30**（DB 没更新）。

**根因**：v1.0.85 同步逻辑用 `result?.available_amount ?? null`，但 `rechargeCard` API 返回**无 available_amount 字段**（只有 `{transaction_id, status}`），导致 `newAmt === null` 同步代码直接跳过。

**修复**（`src/routes/cards.js:461-478`）：
```js
...
setTimeout(async () => {
  try {
    ...
    db.prepare('UPDATE cards SET available_amount=?, last_verified=..., verified_status="verified" WHERE card_id=?').run(detail.available_amount, card_id);
  } catch(e) { logger.warn(...); }
}, 1500);
```

**deploy 脚本静默失败 bug 修复**（`scripts/deploy_prod.js:90-95`）：
- v1.0.99.10 第一次部署时 `git fetch origin` 拉取失败（GitHub 同步慢），但脚本没断言 reset 后 HEAD
- 修复：reset 后 `git rev-parse HEAD` + 目标 commit SHA 严格相等比较，不相等直接 throw

**关键发现**：
- SDK 强制 IPv4 在 commit `15cb5c5` (2026-06-01) 就已加，本次问题**不是** IPv4/IPv6
- IPv4 白名单 139.180.188.104 上游已加（用户截图 IP2）
- 真正根因是 v1.0.85 同步代码逻辑缺陷

**充值历史时间线**（XR2067511181878833152）：
- 07:10:16 充 10 美元 → DB 20→30（手动救急）
- 07:44:10 试 5 美元 → 400003 上游拒
- 07:44:16 充 10 美元 → DB 仍 30（旧代码不同步）
- 07:45-07:47 admin 查卡片 sync=true → DB 30→40
- 07:52:55 部署 v1.0.99.10
- 08:00:32 手动 SDK sync 验证 → UPSTREAM=40, DB=40

**代码 commit**：`49a1b49` (code) + `4df1496` (docs/scripts) + `2a13f2f` (deploy fix)
...
### 卡交易记录页 (v1.0.99.52~62)

#### 页面结构
- 移除"卡交易记录 所有虚拟卡的交易明细"标题
- 交易明细改为与账户总览一致的9列表格(卡号/商户/交易类型/授权金额/授权币种/交易状态/结算金额/消费记录ID/交易时间)
- 复用 `ov-tx-head`/`ov-tx-row` CSS，金额配色(绿/红/蓝)一致
- 全量展示(page_size=999)

#### 筛选功能
- 卡号搜索: `cm-search-input-wrap`样式(与卡片管理同款)，前端本地模糊匹配
- 类型下拉: 全部类型/消费授权/清算/退款/撤销
- 状态下拉: 全部状态/清算完成/清算中/交易失败
- 日期选择器(无"交易时间"文字标签)
- 搜索/重置/导出CSV按钮
...
#### 分页
- 每页10条，pill药丸样式(与卡片管理一致)
- 容器min-height:600px + flex布局，数据少不缩小
- 分页器固定底部居中

#### CSV导出
- 前端从 `_cardTxFiltered` 生成CSV(与页面筛选数据一致)
- Safari兼容: POST /api/csv-proxy换token→window.open GET下载
- fallback: csv-proxy失败用Blob下载

### 用户侧边栏优化 (v1.0.99.63~69)

#### 申请开卡页
- 移除"申请开卡 选择卡段，填写信息，提交审批"标题
...
#### 用户信息弹窗
- 移除"ID #3"显示
- 底部按钮: 灰色"关闭"→渐变色 `var(--grad)` + "退出登录" + `doLogout()`
