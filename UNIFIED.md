...
- **真实原因**：vmcardio 上游**不允许带余额 active 卡**删除（与 v1.0.99.1 用户口述的"自动退余额"不符）

### ⚠️ 业务规则更正
> v1.0.99.1 假设**被推翻**：vmcardio 上游 `deleteCard` API **不允许带余额 active 卡**删除（错误码 700013）。
> 用户卡内余额 $20 → 上游拒绝删除 → **不存在** "自动退余额"功能。
> **项目无退款 API**（SDK 有 `refundCard` 但前端未暴露），用户卡内余额清零只能：
> 1. 消费完卡内余额
> 2. 联系 vmcardio 上游客服退款
> 3. **未来可能**：前端加"退款"按钮（需业务决策）
