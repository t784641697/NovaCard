...
| v1.0.99.39 | 2026-06-25 | **流水弹窗已删除卡状态标签**：绿色"deleted"→红色"已注销" |
| v1.0.99.40 | 2026-06-25 | **普通用户卡片管理搜索+导出+卡号显示**：(1) 移除顶部标题和申请新卡按钮；(2) 新增搜索栏(卡号搜索+状态筛选+日期选择器)；(3) 导出CSV(关联筛选条件,Safari兼容csv-proxy)；(4) 卡号前4位不遮挡:`5331 **** **** 1127` |
| v1.0.99.41 | 2026-06-25 | **卡片管理开卡时间+分页**：(1) 有效期下方新增`开卡时间：yyyy/mm/dd hh:mm:ss`；(2) 用户模式分页每页5条+pill样式分页控件 |
| v1.0.99.42 | 2026-06-25 | **开卡时间格式+容器固定+分页样式**：(1) 去掉.011Z后缀；(2) 卡片列表容器固定640px高度；(3) 分页改为pill药丸样式 |
| v1.0.99.43 | 2026-06-25 | **分页器固定底部+箭头3倍放大**：flex布局分页器固定+箭头48×48px/24px字体 |
| v1.0.99.44 | 2026-06-25 | **卡片导出CSV路径修复**：apiFetch('/api/csv-proxy')路径重复/api→改为/api(csv-proxy) |
| v1.0.99.45 | 2026-06-25 | **持卡人姓名写入cards表**：cards表新增first_name/last_name列；审批时写入；回填脚本7/7成功；导出CSV不再显示"Virtual Card" |
| v1.0.99.46 | 2026-06-25 | **导出CSV增加CVV和账单地址列** |
| v1.0.99.47 | 2026-06-25 | **账户总览交易记录字段优化**：卡BIN→完整卡号；移除卡产品列；交易类型改为预授权/结算/退款/撤销；交易金额→结算金额；新增授权金额/授权币种/消费记录ID列 |
| v1.0.99.48 | 2026-06-25 | **账户总览交易记录为空修复**：前端过滤条件status=success与API返回COMPLETE格式不匹配→改为status=COMPLETE+type=Authorization；后端page_size上限200→999 |
| v1.0.99.49 | 2026-06-25 | **交易列顺序调整**：交易类型→授权金额→授权币种→交易状态→结算金额 |
| v1.0.99.50 | 2026-06-25 | **账户总览自动同步上游交易**：用户访问/transactions时自动触发vmcardio同步(10分钟去重+15秒超时)；修复变量名router→transactionsRoute |
| v1.0.99.51 | 2026-06-25 | **交易记录样式优化**：(1) 金额按状态配色(已完成绿/失败红/清算中蓝)；(2) 去掉+号；(3) 卡号缩略5331****1127；(4) 列间距padding 6px 14px；(5) 统一字号.8rem |
| v1.0.99.52 | 2026-06-26 | 卡有效期隐藏(**/**)+卡交易记录页优化(移除标题+卡片改卡号+中间8位*) |
| v1.0.99.53 | 2026-06-26 | 卡交易记录改9列表格(与账户总览一致)+全量展示(page_size=999) |
| v1.0.99.54 | 2026-06-26 | 卡交易记录加交易状态筛选(COMPLETE/PENDING/DECLINED)+移除"交易时间"文字 |
| v1.0.99.55 | 2026-06-26 | 类型/状态筛选从按钮tab改下拉列表(`.cm-filter-select`) |
| v1.0.99.56 | 2026-06-26 | 卡交易记录加卡号搜索文本输入框+前端本地模糊匹配 |
| v1.0.99.57 | 2026-06-26 | 卡号搜索改为卡片管理同款`cm-search-input-wrap`样式 |
| v1.0.99.58 | 2026-06-26 | **修复卡交易记录加载失败**: 替换搜索框时漏掉`filtered`变量定义→补回卡号筛选逻辑 |
| v1.0.99.59 | 2026-06-26 | 卡号格式改为`5258***0208`(前4+3*+后4) |
| v1.0.99.60 | 2026-06-26 | 卡交易记录加分页(每页10条+pill样式)+容器min-height:600px+分页器固定底部 |
| v1.0.99.61 | 2026-06-26 | CSV改为前端从`_cardTxFiltered`生成(与页面筛选数据一致) |
| v1.0.99.62 | 2026-06-26 | CSV导出Safari兼容(csv-proxy方案)+fallback Blob |
| v1.0.99.63 | 2026-06-26 | 移除申请开卡页"申请开卡 选择卡段，填写信息，提交审批"标题 |
| v1.0.99.64 | 2026-06-26 | 移除用户弹窗"ID #3"显示 |
| v1.0.99.65 | 2026-06-26 | 移除用户弹窗"退出登录"按钮(后v1.0.69以新形式恢复) |
| v1.0.99.66 | 2026-06-26 | 移除侧边栏用户区分隔线(border-top) |
| v1.0.99.67 | 2026-06-26 | 用户区背景色加深(rgba(0,0,0,.15)→后v1.0.68修正) |
| v1.0.99.68 | 2026-06-26 | `.user-chip`背景色修正: rgba(126,184,247,.06)/hover .12 |
| v1.0.99.69 | 2026-06-26 | 用户弹窗按钮改渐变色(var(--grad))+"退出登录"+doLogout() |
| v1.0.99.70 | 2026-06-28 | 卡片管理导出按钮改渐变色样式(与账户流水一致) |
| v1.0.99.71 | 2026-06-28 | 管理员用户管理-移除标题+加筛选(搜索/状态/日期) |
| v1.0.99.72 | 2026-06-28 | 管理员用户管理筛选区独立div+搜索框/日期选择器与卡片管理同款 |
| v1.0.99.73 | 2026-06-28 | 管理员用户管理-日期placeholder+防位移(min-width) |
| v1.0.99.74 | 2026-06-28 | 管理员用户管理-移除已锁定选项+搜索按钮右对齐防位移 |
| v1.0.99.75 | 2026-06-28 | 管理员用户管理筛选项布局优化(等间距+弹性宽度) |
| v1.0.99.76 | 2026-06-28 | 管理员用户管理-新增解锁按钮(锁定用户可手动解锁)+日期筛选确认 |
| v1.0.99.77 | 2026-06-28 | 管理员用户管理日期筛选改UTC+8时区修正 |
...
| v1.0.99.83 | 2026-07-20 | **用户#3余额二次修正 $74.80→$94.80**：v1.0.99.82 补录4笔退款后余额偏高，复查发现3个遗留问题：(1) **#24 管理员扣款$50→$30**：原扣款含XR2067511181878833152幽灵充值$30+合法退款$20，合法部分不应扣，改为仅扣幽灵$30；(2) **#45 退款$21→$22**：app_rejected:17:G5237OH 拒绝退款少退1美元开卡费（历史bug，v1.0.94之前退款不含fee_amount），补足；(3) **#89 退款$21→$20**：2069793414578311169(S5331GL)已删卡退款应为卡内余额$20（不含开卡费$1），修正；最终余额$94.80=充值$107.80-10张已删卡开卡费$13，流水总和一致 ✅ |
| v1.0.99.84 | 2026-07-20 | **管理员"查看消费"自动同步上游交易**：`GET /api/admin/users/:id/transactions` 从同步改为 async，加入 vmcardio `cardTransaction` 自动同步（10分钟去重+15秒超时），与用户端 `/api/transactions` 逻辑一致；修复 wod28395@gmail.com 消费记录不可见 |
| v1.0.99.85 | 2026-07-20 | **财务中心用户余额分布修复**：(1) `topup_total` SQL 从 `type='充值'` 改为 `type IN ('充值','管理员充值')`；(2) `total_spend` SQL 从 `type='消费'` 改为 `type IN ('消费','管理员扣款')`；(3) 新增 `total_refund` 统计 `type='退款'`；(4) 前端新增退款列+绿色样式 |
| v1.0.99.86 | 2026-07-20 | **用户管理默认筛选'正常'状态**：`_auState.status` 初始值从空改为 `'active'`，select 默认选中"正常"，重置按钮也恢复为"正常"筛选 |
