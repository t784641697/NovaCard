...
| v1.0.6 | 2026-05-18 | **修复开卡审批传参**: (1) `product_code` 顺序调为 `app.product_code || app.card_bin`（之前 card_bin 优先，错把 BIN 号当 product_code 传）；(2) 补充 `user_id: '20098106'` 参数（vmcardio 的"卡关联用户"）；(3) 持卡人姓名自动去除数字；(4) 前端各文件同步添加姓名数字校验 |
